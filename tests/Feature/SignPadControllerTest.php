<?php

use Illuminate\Support\Facades\Config;
use Kaemmerlingit\LaravelSignPad\Tests\Models\TestModel;
use Kaemmerlingit\LaravelSignPad\Tests\TestClasses\TestSignature;
use Illuminate\Http\Response;
use Illuminate\Support\Facades\Storage;

it('has an URL to sign the document', function () {
    $this->post(route('sign-pad::signature'))
        ->assertStatus(Response::HTTP_FOUND);
});

it('all parameters are needed', function () {
    $this->post(route('sign-pad::signature'))
        ->assertSessionHasErrors();
});

it('validates the data', function () {
    $this->post(
        route('sign-pad::signature'),
        [
            'model' => 'TestModel',
            'sign' => app(TestSignature::class),
            'id' => 1,
            'token' => md5(config('app.key') . 'TestModel'),
        ]
    )
        ->assertSessionHasNoErrors()
        ->assertStatus(Response::HTTP_INTERNAL_SERVER_ERROR);
});

it('stores the signature image and deletes', function () {
    Storage::fake(config('sign-pad.disk_name'));
    $model1 = TestModel::create();
    $model2 = TestModel::create();
    $sign = 'data:image/png;base64, ◀data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAlgAAADICAYAAAA0n5+2AAAAAXNSR0IArs4c6QAAEG1JREFUeF7t3U2odVUZB/C/BObMV6pBkOhAjXJgIYSRYk2CBqERUY3UCo2w1EkfkzSCqElpkG';

    $this->post($model1->getSignatureRoute(), ['sign' => $sign])->assertStatus(201);;
    $this->post($model2->getSignatureRoute(), ['sign' => $sign])->assertStatus(201);;

    /** @var \Kaemmerlingit\LaravelSignPad\Signature $signature1 */
    $signature1 = $model1->signatures->first();
    $this->assertNotNull($signature1);
    /** @var \Kaemmerlingit\LaravelSignPad\Signature $signature2 */
    $signature2 = $model2->signatures->first();
    $this->assertNotNull($signature2);

    Storage::disk(config('sign-pad.disk_name'))->assertExists($filePath = $signature1->getSignatureImagePath());

    $content = Storage::disk(config('sign-pad.disk_name'))->get($filePath);
    $decodedImage = base64_decode(explode(',', $sign)[1]);
    $this->assertSame($decodedImage, $content);

    $signature1->delete();
    $this->assertFileDoesNotExist($filePath);
    Storage::disk(config('sign-pad.disk_name'))->assertMissing($filePath);
    Storage::disk(config('sign-pad.disk_name'))->assertExists($signature2->getSignatureImagePath());
});

it('stores the encrypted signature image and deletes', function () {
    Storage::fake(config('sign-pad.disk_name'));
    Config::set("app.key", "base64:" . base64_encode(random_bytes(32)));
    Config::set('sign-pad.encrypt_files', true);
    $model1 = TestModel::create();
    $model2 = TestModel::create();
    $sign = 'data:image/png;base64, ◀data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAlgAAADICAYAAAA0n5+2AAAAAXNSR0IArs4c6QAAEG1JREFUeF7t3U2odVUZB/C/BObMV6pBkOhAjXJgIYSRYk2CBqERUY3UCo2w1EkfkzSCqElpkG';

    $this->post($model1->getSignatureRoute(), ['sign' => $sign, 'part' => 'another'])->assertStatus(201);
    $this->post($model1->getSignatureRoute(), ['sign' => $sign, 'part' => 'sepa'])->assertStatus(201);
    $this->post($model2->getSignatureRoute(), ['sign' => $sign])->assertStatus(201);

    /** @var \Kaemmerlingit\LaravelSignPad\Signature $signature1 */
    $signature1 = $model1->signatures()->part('sepa')->first();
    $this->assertNotNull($signature1);
    $this->assertSame($signature1->part, 'sepa');

    /** @var \Kaemmerlingit\LaravelSignPad\Signature $signature2 */
    $signature2 = $model2->signatures->first();
    $this->assertNotNull($signature2);

    Storage::disk(config('sign-pad.disk_name'))->assertExists($filePath = $signature1->getSignatureImagePath());

    $content = $signature1->getSignatureImageContent();
    $decodedImage = base64_decode(explode(',', $sign)[1]);
    $this->assertSame($decodedImage, $content);

    $signature1->delete();
    $this->assertFileDoesNotExist($filePath);
    Storage::disk(config('sign-pad.disk_name'))->assertMissing($filePath);
    Storage::disk(config('sign-pad.disk_name'))->assertExists($signature2->getSignatureImagePath());
});
